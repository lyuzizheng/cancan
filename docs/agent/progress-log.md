...
- Finish independent review and the full native gate for the active `vault-manual-import` image checkpoint. The next candidate is the evidence-only `local-inbox-readiness` investigation, which remains blocked from production folder behavior by Local Inbox filesystem readiness; `review-ledger-ui` remains blocked by the component foundation and Figma role.
