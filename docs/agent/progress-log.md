...
- Continue with slice 3 (Command Center with the final `0006` Tasks IA: unified Tasks section ≤5 rows, full Tasks route with Needs action / In progress / Recently completed / Parked filters, exact owner deep links with return-context, sidebar Tasks entry replacing inert Jobs — renderer-only over the frozen host Tasks read contract, plus the `View document` secondary action's document id on the frozen source-confirmation prompt presentation type), then slice 4 Sources/Intake, slice 5 Review, slice 6 polish with the final designer-level review routed to Kimi Code CLI. Then the remaining `phase1-intake-experience` checkpoints: bounded saved-password iteration limit, canonical-content duplicate handling, phone Shortcut, opt-in batch notification delivery, and menu-bar/notification reopen paths for the background window.
