...
- Added `local-inbox-automation` after the core review UI and before Gmail, so watched-folder reliability, routing, and coverage are proven without OAuth. Kept the future native iOS Share Extension as a thin intake companion behind its own transport/encryption/lifecycle/release feasibility slice.
