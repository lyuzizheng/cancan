...
- Continue with slice 2 (app shell: Vault Spine + Ledger header + right-side Ledger layout optimization + Vault gate on the new foundation), then slice 3 Command Center with the final `0006` Tasks IA (unified Tasks section, full Tasks route, owner deep links — renderer-only over the frozen host Tasks read contract), slice 4 Sources/Intake, slice 5 Review, slice 6 polish with the final designer-level review routed to Kimi Code CLI. Then the remaining `phase1-intake-experience` checkpoints: bounded saved-password iteration limit, canonical-content duplicate handling, phone Shortcut, opt-in batch notification delivery, and menu-bar/notification reopen paths for the background window.
