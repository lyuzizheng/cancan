...
- Implemented the Kimi-led `public-project-surface` work on its feature branch under that recorded exception. Added `apps/website` (`@cancan/website`): the static public site as React views prerendered to plain HTML per route (Overview, Download, Privacy, Security, Docs, 404), sharing the `0011` visual tokens, with schematic CSS mockups as placeholders for later real app screenshots, no backend/analytics/trackers, and honest pre-release copy (contacts intended but unprovisioned, no signed artifacts yet, no unverified x86_64 claim, Gmail pending Google verification). Wired `pnpm build:website` and `pnpm check:website` (build plus deterministic link/required-disclosure/accessibility-structure check over `dist/`) into `pnpm verify:fast` and `pnpm verify`, and registered the gate in `0001`/`0016`.
