...
- Implement the dedicated local connector execution route without changing the frozen adapter or persistence contracts: random loopback binding with bounded timeout, external browser opening, and injected HTTP transport. Keep tokens and the PKCE verifier outside renderer/Tauri presentation responses; do not add rules, sync, or live credentials in that checkpoint.
