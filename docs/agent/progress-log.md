...
- Fixed Gmail connection behavior to one read/configured-AI-processing authorization per mailbox, no repeated rule-level consent, globally serial rule sync, and disconnect that disables rules, cancels/stops sync before more capture, deletes the Keychain token, and retains mailbox/rule/cursor/evidence identities for reconnect.
