...
- The `synthetic-core-flow` slice is complete. Its schema and repository cover only exercised synthetic queries. `vault-manual-import` is now in progress: the production Rust host includes the canonical migrations, opens SQLCipher with a context-separated raw database subkey, writes `CCENV001` files, and coordinates exact-hash import outcomes with store-open/rollback recovery logic. App startup now owns a Rust-only locked/unlocked Vault session; the accepted Argon2id password wrapper recovers the master key without returning it to the renderer. Creation prepares and syncs an inactive candidate before rename, rolls back handled activation failures, and can reopen an activated Vault after restart. An abrupt pre-activation process crash may leave an inactive `.vault-create-*` directory; automatic cleanup is deferred until CanCan has a cross-process ownership/locking protocol rather than risking deletion beneath a second app process. The host now exposes status/create/unlock/lock plus host-picked PDF/CSV import and safe per-source document-list commands. File paths, encrypted locators, hashes, and bytes do not cross into renderer responses; import/list require an unlocked Vault, and initial import stores null semantic identity rather than accepting it from the renderer. Parser, trusted semantic classification, Money Source/Source/Documents UI wiring, viewer, Keychain remembering, recovery-file action, system/inactivity lock hooks, statement-password, and deletion wiring remain in this slice.
