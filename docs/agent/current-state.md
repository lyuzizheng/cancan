...
- The `synthetic-core-flow` slice is complete. Its schema and repository cover only exercised synthetic queries. `vault-manual-import` is now in progress: the production Rust host includes the canonical migrations, opens SQLCipher with a context-separated raw database subkey, writes `CCENV001` files, and coordinates exact-hash import outcomes with store-open/rollback recovery logic. Application-startup command/state wiring, parser, Source/Documents UI, viewer, statement-password, and deletion wiring remain in this slice.
...
Close the current encrypted-storage/design-contract checkpoint without implementing the redesigned UI. After review and merge, continue `vault-manual-import` by exposing only narrow Tauri commands, normalizing a synthetic/redacted statement through the selected mock sidecar boundary, and implementing Source/Documents against the accepted design contract before adding viewer, statement-password, deletion, and recovery UX. Do not broaden this slice into Gmail OAuth, public release, backup scheduling, or real `x86_64` qualification. A real `x86_64` build/runtime/security pass remains mandatory in `backup-release` before Phase 1 ships. Run the second public-launch grill after the review/ledger UI and before public OAuth/release work.
