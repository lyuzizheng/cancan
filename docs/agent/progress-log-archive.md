...
Historical dated entries moved out of `docs/agent/progress-log.md` to keep the active log short. These are historical context only; the active `docs/agent/progress-log.md` (recent entries) and `docs/agent/current-state.md` are authoritative for current state.
