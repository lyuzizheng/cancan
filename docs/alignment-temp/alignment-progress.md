...
| Local Inbox filesystem readiness | blocked | [Ordinary-local and opt-in synthetic iCloud evidence](../../spikes/local-inbox-readiness/EVIDENCE.md) proves stable handle-bound capture, deletion suppression, fresh rescan recovery, fail-closed paths, upload-ready state, and one real `brctl evict`/`not-downloaded` path. Rust capture then deferred without bytes but the status became `current` during capture. It does not prove pre-hydration placeholder deferral, app/process-restart lifecycle, or a settle interval; keep the blocker until a native preflight-before-open design and evidence exist | `docs/specs/0017-evidence-documents-source-ux.md`, `docs/specs/0016-testing-fixtures-agent-gates.md` |
