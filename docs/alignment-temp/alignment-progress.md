...
| Vault/file/backup security validation | blocked | Finalize production Argon2id parameters, authenticated-file and recovery formats, cross-platform secret-store behavior, temporary-plaintext cleanup, same-model backup recovery, and atomic restore; primitive feasibility and the three-concept user model are accepted | `docs/specs/0009-backup-restore-versioning.md` and an ADR if architecture changes |
...
| Document normalizer runtime evidence | blocked | Compare Vercel AI SDK ToolLoopAgent with Pi Agent Core under the same fixed-tools/mock-model harness; prove the Node/Tauri worker or sidecar packaging, cancellation, secret boundary, release size/signing, and deterministic fixture path. Full Pi Coding Agent is excluded and an OS sandbox is optional rather than assumed | `docs/specs/0001-repo-structure.md`, `docs/specs/0004-parser-contract.md`, `docs/specs/0016-testing-fixtures-agent-gates.md`, plus an ADR if the accepted runtime changes architecture |
| Build and open-source release details | partial | Decide MVP operating systems, project license, community governance/maintainer authority, signing/notarization identities, updater-key custody, stable/beta and security-update policy, release approval, immutable-release policy, and whether independently signed parser packs are ever allowed; GitHub Releases, semantic versions, signed artifacts, provenance/SBOM evidence, and no proprietary updater backend are accepted | `docs/specs/0018-app-updates-open-source-release.md` |
...
| Security observability and sensitive-data lifecycle | partial | Decide log redaction, opt-in crash-report details, full-document extraction/index retention, and deletion behavior. Bounded validated raw source objects per external record are accepted; MVP analytics/behavioral telemetry is excluded | future focused security spec after the decision |
