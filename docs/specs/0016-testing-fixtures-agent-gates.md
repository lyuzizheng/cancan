...
The production skeleton and package scripts now exist. `.github/workflows/application.yml` runs repository preflight and `pnpm verify:fast` on Linux with frozen pnpm resolution for desktop-application changes (`apps/desktop/**`, `packages/**`, and root toolchain files) on pull requests and `main`; it does not trigger on the isolated static website. `verify:fast` still runs `pnpm check:website` so a shared root or dependency change that reaches the site still fails the fast path. The static website is otherwise owned by a dedicated Linux workflow, `.github/workflows/website.yml`, triggered only by `apps/website/**` and its build toolchain: because `@cancan/website` has no workspace dependencies, a static-site copy change pays only the website's own `pnpm --filter @cancan/website typecheck` plus `pnpm check:website` (build plus deterministic link, required-disclosure, and accessibility-structure checks over the emitted `dist/`) instead of the full desktop typecheck, unit, and build chain. The website typecheck is explicit because the Vite build strips types without checking them. Every application, website, native, spike-runtime, and vault workflow cancels superseded runs per pull request or ref so rapid pushes do not accumulate redundant paid minutes. `.github/workflows/application-native.yml` runs repository preflight and `pnpm verify:native` with the pinned macOS toolchain and frozen pnpm/Cargo resolution only for ready native, connector/sidecar/parser, migration, dependency, or toolchain changes, with manual dispatch available. The native Rust gate restores bounded Cargo cache data and builds the document-normalizer and dedicated Gmail connector sidecars once before executing privileged encrypted-file, SQLCipher, import rollback, tombstone, loopback-boundary, clippy, and desktop-build checks in authoritative CI; standalone commands remain self-contained. The root TypeScript unit gate excludes `spikes/**`; disposable spike tests run only through their isolated package/workflow gates and dependencies. The production TypeScript unit-test gate includes the synthetic core's safe-reset, repeatable-migration, deterministic parser, exact reconciliation, review, commit, read-model, relationship, and mocked Gmail connector-protocol tests; `pnpm test:synthetic-core` exposes the same focused financial subset locally. The docs-harness CI remains outside this spec's ownership.
