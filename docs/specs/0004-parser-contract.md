...
For PDF input, native text extraction is always attempted because it is local and cheap. OCR runs for scanned or broken pages, weak numeric/layout coverage, or evidence-grounding gaps.

The implemented local macOS Vision subset pins `VNRecognizeTextRequest` revision 3 with accurate recognition and runs per page only when native text is empty/whitespace or has a clear broken-text marker: NUL, U+FFFD, or a non-layout control character. Layout controls are tab, LF, vertical tab, form feed, CR, and NEL. It does not yet infer weak quality from text length, numeric/layout coverage, or evidence-grounding outcomes. It preserves the native observation and appends separate OCR observations. If the page has no usable native content, a render or OCR failure, including a missing Vision result collection, rejects the extraction bundle. If usable non-empty native text remains alongside a broken marker, failed supplemental OCR is omitted and the native observation continues to downstream grounding and validation; it is never relabeled as OCR. Broader input planning remains accepted future work.

CSV and other structured exports use deterministic table extraction before AI normalization. A multimodal normalizer may also receive the original PDF or page images.
