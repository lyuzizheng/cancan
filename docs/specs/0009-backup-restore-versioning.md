...
One optional statement-PDF password may be saved per Money Source in macOS Keychain. SQLite stores only its secret reference and status. When the source is not yet known, the privileged host may try every distinct saved statement password once for that parse attempt. That pass is host-owned and fixed: it walks the saved references in a stable Money Source order, probes each distinct secret at most once for the attempt, and stops at the first unlock, so no renderer input selects or orders the attempts and no attempt repeats. Every secret stays under the Money Source that stored it; the pass never moves or rewrites ownership, and the renderer receives only the outcome. A successful password only unlocks bytes; it never identifies the Money Source. If none works, CanCan asks for a password. After trusted classification and any required source confirmation, it offers `Use once`, `Save for this source`, or `Update saved password`; MVP stores no password history and no unlocked duplicate of the PDF.
