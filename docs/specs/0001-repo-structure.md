...
Phase 1 uses one Tauri process, not a separate helper service. Closing the final window destroys the React renderer/WebView while a hidden macOS background `WebviewWindow` keeps the Rust runtime alive, retaining the Vault session, SQLCipher/key state, Local Inbox watcher, and event-driven durable-job scheduler. Reopening from the Dock, the menu-bar item, or a background-intake notification recreates the renderer from host/SQLite state while the process still holds the Vault session key, so no re-authentication is asked; a notification click then lands on the Tasks row its batch produced. After a manual lock or a relaunch the key is gone, and the same reopen shows the unlock gate before resuming that route. The process must remain visibly running through normal macOS affordances.
