...
**Operating bias — act.** These gates and the issue loop exist to make autonomous delivery safe, not to require human sign-off at each step. For clear, in-scope work: find or file the issue, pick the slice, make the smallest complete change, and run the tier's gates without pausing for confirmation. A missing issue for obvious in-scope work is filed as part of the same PR flow — never a reason to stall. Stop and ask only when a decision genuinely falls in the ask-list: money/ledger correctness; irreversible migrations or data; security, privacy, or secrets; AI authority; provider-support claims; visual identity; or backup/restore compatibility.
