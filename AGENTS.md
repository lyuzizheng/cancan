...
9. Frontend implementation is owned by Kimi Code CLI. Frontend code means the `apps/desktop` renderer and `packages/ui`; renderer and UI work governed by the visual design specs (`docs/specs/0006-command-center-ui.md`, `docs/specs/0008-design-system.md`, `docs/specs/0011-visual-design-tokens.md`, `docs/specs/0017-evidence-documents-source-ux.md`) must be implemented only through Kimi Code CLI. Any other agent must not implement or restyle frontend code: when a task reaches frontend implementation, stop, tell the user to run that work through Kimi Code CLI, and continue only with the non-frontend parts.
