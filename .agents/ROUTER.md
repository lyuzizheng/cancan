...
| Implement, build, change a slice, 实现, 构建, 变更 | — | [implement-feature](./workflows/implement-feature.md) |
...
| Connector, Gmail, source integration, 数据源 | — | [plugin-work](./workflows/plugin-work.md) |
| Orientation, status, plan, 进度, 规划 | — | [development-cycle](./workflows/development-cycle.md) |
...
Rows marked `—` have no dedicated skill; enter them directly through the workflow (orientation is also covered by `AGENTS.md`). Skills exist only where a distinct discovery trigger earns its keep.
