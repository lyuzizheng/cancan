...
export function createApi(overrides: Partial<VaultApi> = {}) {
  const api = {
    ...
    getMoneySourceDetail: vi.fn(async (moneySourceId: string): Promise<MoneySourceDetail> => ({
    ...
    listSupportedMoneySourceProviders: vi.fn(
      async (): Promise<SupportedMoneySourceProviderSummary[]> => [],
    ),
    createMoneySource: vi.fn(async (): Promise<MoneySourceSummary> => moneySource),
    editMoneySource: vi.fn(async (): Promise<MoneySourceSummary> => moneySource),
    ...
    removeStatementPassword: vi.fn(async (): Promise<void> => undefined),
