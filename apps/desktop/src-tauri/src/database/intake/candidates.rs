        ...
        let rows = statement
            .query_map([], |row| {
                Ok(SourceConfirmationPrompt {
                    ...
                    latest_document_title: row.get(7)?,
                })
            })?
            .collect::<Result<Vec<_>, _>>()?;
        Ok(rows)
